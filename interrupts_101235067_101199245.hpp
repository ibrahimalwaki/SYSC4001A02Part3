...
struct PCB{
    ...
    PCB(unsigned int _pid, int _ppid, std::string _pn, unsigned int _size, int _part_num, unsigned int _rem_cpu = 0):
        PID(_pid), PPID(_ppid), program_name(_pn), size(_size), partition_number(_part_num), remaining_cpu_time(_rem_cpu) {}
};
...
// Helper function to generate a random duration (1-10)
int random_duration() {
...
// Helper function to get the next pid value
unsigned int get_next_pid() {
    return next_pid++;
}

// Helper function to create the child PCB
PCB create_child_pcb(const PCB& parent) {
    return PCB(
        ...
        parent.program_name, 
        parent.size, 
        -1, // Temp value that is changed in the next line of the cpp
        parent.remaining_cpu_time
    );
}


// Helper function to call scheduler
...
// Had an issue with the partitions so added a helper function to fix that issue by freeing the memory and removing the PCB from the list by PID
void cleanup_process(PCB& process, std::vector<PCB>& pcb_list) {

    if (process.partition_number != -1) {
        free_memory(&process);
    }
    
    auto it = std::find_if(pcb_list.begin(), pcb_list.end(),[&process](const PCB& p) { return p.PID == process.PID; });
...
// System Status Log Helpers
std::string print_memory_status() {
    std::stringstream buffer;
    buffer << "Memory Status:\n";

    for (int i = 0; i < 6; i++) {
         buffer << memory[i].partition_number << " | " << memory[i].size << " | " << memory[i].code << "\n";
    }
    return buffer.str();
}

std::string log_system_status(int current_time, PCB current, std::vector<PCB> wait_queue, const std::string& trace_line) {
    std::stringstream ss;
    
    ss << "time: " << current_time << "; current trace: " << trace_line << "\n";
    
    ss << print_PCB(current, wait_queue); 
